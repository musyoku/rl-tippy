...
class Trainer(TippyAgent):
	def __init__(self, dqn, optimizer, replay_frames, agent_history_length):
		super().__init__()
		self.dqn = dqn
		...
		self.last_state = np.zeros((agent_history_length, OBSERVATION_HEIGHT, OBSERVATION_WIDTH), dtype=np.float32)
	...
	def agent_step(self, action, reward, next_frame, score):
		...
		return_action = ACTION_NO_OP

		if self.time_step > self.agent_history_length:
...
def run_training_loop():
	...
	agent = Trainer(dqn, optimizer, replay_frames, conf.agent_history_length)
	agent.play()
...
if __name__ == "__main__":
	...
	save_config(os.path.join(sandbox, "conf.json"), conf)
	run_training_loop()